...
/**
 * @brief Structure to represent adjacency map for multigraph
 */
struct AdjacencyMap {
...
/**
 * @brief Read adjacency map from edge list file for multigraph
...
/**
 * @brief Convert adjacency list to adjacency map with self-loop removal
...
AdjacencyMap convertAdjacencyListToMap(const std::vector<std::vector<int>>& adjacencyData, int numberOfVertices) {
    ...
    if (selfLoopCount > 0) {
        std::cout << "Warning: " << selfLoopCount << " self-loops removed during conversion to multigraph map" << std::endl;
    }
...
/**
 * @brief Convert extended adjacency list to adjacency map with self-loop removal
...
void displayAdjacencyMap(const AdjacencyMap& adjacencyMap) {
    std::cout << "=== Adjacency Map (MultiGraph) ===" << std::endl;
...
void writeAdjacencyMapToFile(const AdjacencyMap& adjacencyMap, const std::string& fileName) {
    ...
    outputFile << "=== Adjacency Map (MultiGraph) ===" << std::endl;
...
/**
 * @brief Convert adjacency matrix to adjacency map with self-loop removal
