...
/**
 * @brief Demonstrate all 12 conversion functions between graph representations for multigraph
 */
void demonstrateMultiGraphRepresentationConversions() {
    const std::string INPUT_FILE_NAME = "input.txt";
    
    std::cout << "=== Complete MultiGraph Representation Conversion Demo ===" << std::endl;
    std::cout << "Demonstrating all 12 possible conversions between 4 representations" << std::endl;
    std::cout << "Note: Self-loops are automatically detected and removed" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "Graph type: MultiGraph" << std::endl;
    std::cout << "All 12 conversions completed successfully!" << std::endl;
}

/**
 * @brief Test conversion functions with matrix input format for multigraph
 */
void testMultiGraphMatrixInputFormat() {
    std::cout << "\n=== Testing MultiGraph Matrix Input Format ===" << std::endl;
    
    // Create matrix format input with self-loops to test removal
    std::ofstream matrixFile("matrix_input.txt");
    matrixFile << "4" << std::endl;
    matrixFile << "1 1 1 0" << std::endl;  // Self-loop at (0,0)
    matrixFile << "0 2 0 1" << std::endl;  // Self-loop at (1,1) with 2 edges
    matrixFile << "0 0 1 0" << std::endl;  // Self-loop at (2,2)
    matrixFile << "1 0 0 3" << std::endl;  // Self-loop at (3,3) with 3 edges
    matrixFile.close();
    
    AdjacencyMatrix matrixFromFile = readAdjacencyMatrixFromFile("matrix_input.txt");
    std::cout << "Matrix read from file (after self-loop removal):" << std::endl;
...
int main() {
    ...
    try {
        demonstrateMultiGraphRepresentationConversions();
        testMultiGraphMatrixInputFormat();
        
        std::cout << "=== MultiGraph Representation Demo Completed Successfully ===" << std::endl;
