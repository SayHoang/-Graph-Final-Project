...
/**
 * @brief Applies Dijkstra's algorithm to find shortest paths from source vertex in multigraph
...
/**
 * @brief Validates that edge is not a loop (not allowed in multigraph)
 * @param firstVertex First vertex of the edge
 * @param secondVertex Second vertex of the edge
 * @return True if edge is valid (not a loop)
 */
...
/**
 * @brief Reads graph input from file with loop validation for multigraph
 ...
 * @return Adjacency list representation of the multigraph
 */
vector<vector<DistanceVertexPair>> readMultigraphInput(ifstream& inputFile, int& numberOfVertices, int& numberOfEdges) {
    ...
    for (int i = 0; i < numberOfEdges; ++i) {
        ...
        if (!isValidEdgeForMultigraph(firstVertex, secondVertex)) {
            cout << "Warning: Loops are not allowed in multigraph. Skipping edge (" 
...
void displayResults(const DijkstraResult& result, int sourceVertex) {
    cout << "Shortest paths from vertex " << sourceVertex << " (Multigraph - no loops, multiple edges allowed):\n";
...
int main() {
    ...
    vector<vector<DistanceVertexPair>> adjacencyList = readMultigraphInput(inputFile, numberOfVertices, numberOfEdges);
