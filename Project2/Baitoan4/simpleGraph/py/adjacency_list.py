"""
Adjacency List implementation for Simple Graph
...
@dataclass
class AdjacencyList:
    """Structure to represent adjacency list for simple graph"""
...
def read_adjacency_list_from_file(file_name: str) -> AdjacencyList:
    """
    Read adjacency list from edge list file with self-loop and duplicate edge removal
...
def display_adjacency_list(adjacency_list: AdjacencyList) -> None:
    ...
    """
    print("=== Adjacency List (Simple Graph) ===")
...
def write_adjacency_list_to_file(adjacency_list: AdjacencyList, file_name: str) -> None:
    ...
    try:
        with open(file_name, 'w') as output_file:
            output_file.write("=== Adjacency List (Simple Graph) ===\n")
...
def convert_matrix_to_adjacency_list(matrix_data: List[List[int]], 
    ...
    """
    Convert adjacency matrix to adjacency list with simple graph validation
    ...
    if self_loop_count > 0:
        print(f"Warning: {self_loop_count} self-loops removed during conversion to simple graph list")
    if multiple_edge_count > 0:
        print(f"Warning: {multiple_edge_count} multiple edges converted during conversion to simple graph list")
...
def convert_extended_adjacency_list_to_list(outgoing_edge_indices: List[List[int]], 
                                          ...
                                          number_of_vertices: int) -> AdjacencyList:
    """
    Convert extended adjacency list to adjacency list with self-loop and duplicate edge removal
    ...
    if self_loop_count > 0:
        print(f"Warning: {self_loop_count} self-loops removed during conversion to simple graph list")
    if duplicate_edge_count > 0:
        print(f"Warning: {duplicate_edge_count} duplicate edges removed during conversion to simple graph list")
...
def convert_adjacency_map_to_list(outgoing_connections: dict, 
                                number_of_vertices: int) -> AdjacencyList:
    """
    Convert adjacency map to adjacency list with self-loop and duplicate edge removal
