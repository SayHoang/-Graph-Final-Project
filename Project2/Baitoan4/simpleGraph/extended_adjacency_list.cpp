...
/**
 * @brief Structure to represent extended adjacency list for simple graph
 */
...
/**
 * @brief Read extended adjacency list from edge list file for simple graph
...
/**
 * @brief Convert adjacency list to extended adjacency list with duplicate edge and self-loop removal
...
ExtendedAdjacencyList convertAdjacencyListToExtended(const std::vector<std::vector<int>>& adjacencyData, int numberOfVertices) {
    ...
    if (selfLoopCount > 0) {
        std::cout << "Warning: " << selfLoopCount << " self-loops removed during conversion to simple graph extended list" << std::endl;
    }
    if (duplicateEdgeCount > 0) {
        std::cout << "Warning: " << duplicateEdgeCount << " duplicate edges removed during conversion to simple graph extended list" << std::endl;
    }
...
/**
 * @brief Convert adjacency matrix to extended adjacency list with duplicate edge and self-loop removal
...
ExtendedAdjacencyList convertMatrixToExtendedAdjacencyList(const std::vector<std::vector<int>>& matrixData, int numberOfVertices) {
    ...
    if (multipleEdgeCount > 0) {
        std::cout << "Warning: " << multipleEdgeCount << " multiple edges converted to single edges during conversion to simple graph extended list" << std::endl;
    }
...
void displayExtendedAdjacencyList(const ExtendedAdjacencyList& extendedList) {
    std::cout << "=== Extended Adjacency List (SimpleGraph) ===" << std::endl;
...
void writeExtendedAdjacencyListToFile(const ExtendedAdjacencyList& extendedList, const std::string& fileName) {
    ...
    outputFile << "=== Extended Adjacency List (SimpleGraph) ===" << std::endl;
...
/**
 * @brief Convert adjacency map to extended adjacency list with duplicate edge and self-loop removal
