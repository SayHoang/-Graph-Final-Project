"""
Adjacency Map implementation for MultiGraph
...
@dataclass
class AdjacencyMap:
    """Structure to represent adjacency map for multigraph"""
...
def read_adjacency_map_from_file(file_name: str) -> AdjacencyMap:
    """
    Read adjacency map from edge list file with self-loop removal
...
def display_adjacency_map(adjacency_map: AdjacencyMap) -> None:
    ...
    """
    print("=== Adjacency Map (MultiGraph) ===")
...
def write_adjacency_map_to_file(adjacency_map: AdjacencyMap, file_name: str) -> None:
    ...
    try:
        with open(file_name, 'w') as output_file:
            output_file.write("=== Adjacency Map (MultiGraph) ===\n")
...
def convert_adjacency_list_to_map(adjacency_data: List[List[int]], 
    ...
    """
    Convert adjacency list to adjacency map with self-loop removal
    ...
    if self_loop_count > 0:
        print(f"Warning: {self_loop_count} self-loops removed during conversion to multigraph map")
...
def convert_extended_list_to_map(outgoing_edge_indices: List[List[int]],
    ...
    """
    Convert extended adjacency list to adjacency map with self-loop removal
...
def convert_matrix_to_adjacency_map(matrix_data: List[List[int]], 
                                  number_of_vertices: int) -> AdjacencyMap:
    """
    Convert adjacency matrix to adjacency map with self-loop removal
